...
async def show_day_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if supps:
        ...
        for s in supps:
            ...
            lines.append(f"{tick} {s['name']} {s['dose']}{time_str}")
...
async def show_week_plan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, day in enumerate(days):
        ...
        wstr = f" 💪 {workout['name']}" if workout else ""
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """All photos go through unified AI handler"""
    user_id = update.effective_user.id
    ...
    await _process_image(update, context, photo_bytes)
