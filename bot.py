...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting or name.endswith('.zip') or name.endswith('.csv'):
        # Determine what kind of file we expect
        is_zip = name.endswith('.zip') or 'zip' in mime
        is_csv = name.endswith('.csv') or 'csv' in mime

        # If user said /import_health but sent CSV - reject
        if waiting == 'health' and not is_zip:
            await update.message.reply_text("Ожидаю ZIP, а не CSV. Используй /import_food для CSV.", reply_markup=main_keyboard())
            return
        if waiting == 'food' and not is_csv:
            await update.message.reply_text("Ожидаю CSV, а не ZIP. Используй /import_health для ZIP.", reply_markup=main_keyboard())
            return

        # Don't pop waiting - let user send more files in same mode
        ...
        if name.endswith('.zip') or 'zip' in mime:
            ...
            # Buffer this file - don't process yet
