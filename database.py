...
class Database:
    ...
    def find_duplicate_meal(self, user_id: int, date_str: str, product_name: str, meal_type: str = None) -> dict:
        """Find existing meal log on same date for same product. Returns dict or None."""
        with self._conn() as conn:
            try:
                if meal_type:
                    row = conn.execute("""
                        SELECT id, calories, protein, fat, carbs, portion_g, time
                        FROM meal_logs
                        WHERE user_id=? AND date=? AND LOWER(product_name)=LOWER(?) AND meal_type=?
                        ORDER BY id DESC LIMIT 1
                    """, (user_id, date_str, product_name, meal_type)).fetchone()
                else:
                    row = conn.execute("""
                        SELECT id, calories, protein, fat, carbs, portion_g, time, meal_type
                        FROM meal_logs
                        WHERE user_id=? AND date=? AND LOWER(product_name)=LOWER(?)
                        ORDER BY id DESC LIMIT 1
                    """, (user_id, date_str, product_name)).fetchone()
                return dict(row) if row else None
            except Exception:
                return None

    def update_meal_kbju(self, meal_id: int, calories: float, protein: float, fat: float, carbs: float, portion_g: float = None):
        """Update existing meal with averaged values"""
        with self._conn() as conn:
            try:
                if portion_g is not None:
                    conn.execute("""
                        UPDATE meal_logs SET calories=?, protein=?, fat=?, carbs=?, portion_g=?
                        WHERE id=?
                    """, (calories, protein, fat, carbs, portion_g, meal_id))
                else:
                    conn.execute("""
                        UPDATE meal_logs SET calories=?, protein=?, fat=?, carbs=?
                        WHERE id=?
                    """, (calories, protein, fat, carbs, meal_id))
            except Exception:
                pass

    def get_product_portion_stats(self, user_id: int, product_name: str) -> dict:
        """Get all portions of a product across all meal_types — for std portion logic.
        Returns: {count, median_portion, min, max, spread_pct}
        """
        from statistics import median
        with self._conn() as conn:
            try:
                rows = conn.execute("""
                    SELECT portion_g FROM meal_logs
                    WHERE user_id=? AND LOWER(product_name)=LOWER(?) AND portion_g IS NOT NULL
                """, (user_id, product_name)).fetchall()
                portions = [r['portion_g'] for r in rows if r['portion_g'] and r['portion_g'] > 0]
                if not portions:
                    return {'count': 0}
                med = median(portions)
                mn = min(portions)
                mx = max(portions)
                spread = ((mx - mn) / med * 100) if med > 0 else 0
                return {
                    'count': len(portions),
                    'median_portion': round(med, 1),
                    'min': round(mn, 1),
                    'max': round(mx, 1),
    ...
    def get_meals_for_date(self, user_id: int, date_str: str) -> list:
        """Get all meals for a specific date"""
        with self._conn() as conn:
            try:
                rows = conn.execute("""
                    SELECT id, time, description, calories, protein, fat, carbs,
                           portion_g, product_name, meal_type
                    FROM meal_logs WHERE user_id=? AND date=?
                    ORDER BY time
    ...
    def find_similar_meal(self, user_id: int, date_str: str, product_name: str, time_str: str = None) -> dict:
        """Find existing meal that matches by product name (and optionally close time).
        Returns meal dict or None."""
        existing = self.get_meals_for_date(user_id, date_str)
        if not existing:
            return None
        ...
        for m in existing:
            ex_name = (m.get('product_name') or m.get('description') or '').lower()
            if product_lower in ex_name or ex_name in product_lower:
                # Check time proximity if provided
                if time_str and m.get('time'):
                    try:
                        h1 = int(time_str[:2])
                        h2 = int(m['time'][:2])
                        if abs(h1 - h2) <= 1:
    ...
    def get_product_standard_portion(self, user_id: int, product_name: str) -> dict:
        """Returns {portion_g, count, deviation_pct} for a product, or None if not enough data"""
        from statistics import median
        if not product_name:
            return None
        ...
        with self._conn() as conn:
            try:
                rows = conn.execute("""
                    SELECT portion_g FROM meal_logs
                    WHERE user_id=? AND LOWER(COALESCE(product_name, description)) LIKE ?
                          AND portion_g IS NOT NULL AND portion_g > 0
                    ORDER BY date DESC LIMIT 30
                """, (user_id, f'%{name_norm}%')).fetchall()
                portions = [r['portion_g'] for r in rows if r['portion_g']]
                if len(portions) < 2:
                    return None
                med = median(portions)
                # Calculate deviation
                deviations = [abs(p - med) / med * 100 for p in portions]
                max_dev = max(deviations)
                return {
                    'portion_g': round(med, 1),
                    'count': len(portions),
