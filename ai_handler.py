...
class AIHandler:
    ...
    def _date_context(self) -> str:
        ...
        return f"Сегодня: {days_ru[now.weekday()]}, {now.strftime('%d.%m.%Y')}, время: {now.strftime('%H:%M')}."
...
Верни ТОЛЬКО JSON без пояснений."""
        ...
        messages = []
        if chat_history:
            messages.extend(chat_history[-8:])  # last 4 exchanges
...
Все числа без единиц измерения, только цифры."""
    ...
    async def calculate_kbju_unified(
        self,
        inbody: dict,
        profile: dict,
        goal_type: str,
        ...
        """
        Unified KBJU calculation using ALL available data.
        goal_type: 'fat_percent' | 'weight' | 'cut' | 'bulk' | 'maintain'
        goal_value: target fat% or weight if goal_type is specific
        """
        ...
        weight = inbody.get('weight') or profile.get('weight') or 80
        fat_percent = inbody.get('fat_percent') or profile.get('fat_percent')
        lean_mass = inbody.get('muscle_mass')
        bmr_from_report = inbody.get('bmr')
        age = profile.get('age') or 30
        height = profile.get('height') or 175
        gender = profile.get('gender', 'male')
        activity = profile.get('activity', 'moderate')

        # Calculate lean mass if not from InBody
        if not lean_mass and fat_percent:
            lean_mass = weight * (1 - fat_percent / 100)

        # BMR: priority — InBody report > Katch-McArdle > Mifflin
        if bmr_from_report:
            bmr = bmr_from_report
        elif lean_mass:
        ...
        # Determine deficit based on goal
        if goal_type == 'fat_percent' and goal_value and fat_percent:
            # Calculate how much fat to lose
            current_fat_kg = weight * (fat_percent / 100)
            target_fat_kg = weight * (goal_value / 100)
            fat_to_lose = max(0, current_fat_kg - target_fat_kg)
            # Aggressive but safe: ~0.5kg fat/week = 500 kcal/day deficit
            weeks_needed = round(fat_to_lose / 0.5)
            deficit = min(700, max(300, round(fat_to_lose * 100)))
            calories = round(tdee - deficit)
            protein_per_kg = 2.4
            goal_summary = f"цель {goal_value}% жира, ~{weeks_needed} нед"
        elif goal_type == 'weight' and goal_value:
            to_lose = weight - goal_value
            if to_lose > 0:
                deficit = min(700, max(300, round(to_lose * 70)))
                calories = round(tdee - deficit)
                protein_per_kg = 2.2
                goal_summary = f"цель {goal_value} кг"
            else:
                calories = round(tdee * 1.1)
                protein_per_kg = 2.0
                goal_summary = f"набор до {goal_value} кг"
        elif goal_type == 'bulk':
            calories = round(tdee * 1.1)
            protein_per_kg = 2.0
            goal_summary = "набор массы"
        elif goal_type == 'maintain':
            calories = round(tdee)
            protein_per_kg = 1.8
            goal_summary = "поддержание"
        else:  # cut default
            calories = round(tdee * 0.82)
            protein_per_kg = 2.4
            goal_summary = "сушка"

        protein = round((lean_mass or weight) * protein_per_kg)
        ...
        return {
            ...
            "fat_to_lose": round(fat_to_lose, 1) if goal_type == 'fat_percent' and goal_value and fat_percent else None,
        }
    ...
    async def calculate_kbju_from_inbody(
        ...
        """Legacy wrapper — use calculate_kbju_unified instead"""
        profile = {'activity': activity, 'goal': goal}
        goal_type = goal if goal in ('cut', 'bulk', 'maintain') else 'cut'
        result = await self.calculate_kbju_unified(inbody, profile, goal_type)
        return result
